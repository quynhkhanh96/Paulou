...
Converts a raw GOP value to a 0-100 score via per-phone z-score against a
native-speaker reference distribution, then a normal CDF (Decision Log D11).
Rejected a supervised regressor because no labeled French mispronunciation
dataset exists (unlike English's speechocean762).

IMPORTANT — data gap, not yet resolved: the per-phone mean/std table this
module needs is supposed to come from running GOP on a native French corpus
offline (e.g. Common Voice fr, per D11) — that corpus run hasn't happened
yet. `calibrate_score` therefore takes the stats table as a parameter
...
class PhoneStats:
    """Native-speaker reference distribution for one phone's raw GOP score."""
...
def calibrate_score(
    ...
    """Convert a raw GOP value for `phone` into a calibrated 0-100 score.

    Looks up `phone` in `phone_stats`. If its sample_size is below
    `min_sample_size` (unstable mean/std), falls back to the broader phone
    class given by `fallback_phone_class[phone]`, if provided. Raises if no
    usable stats can be found at all.
    """
    ...
    if stats.std == 0:
        ...
        z = (raw_gop - stats.mean) / stats.std
