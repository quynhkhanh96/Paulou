...
def test_raw_value_equal_to_mean_gives_score_50():
    stats = {"a": PhoneStats(mean=0.0, std=1.0, sample_size=100)}
    assert calibrate_score(raw_value=0.0, phone="a", phone_stats=stats) == 50


def test_raw_value_far_below_mean_gives_low_score():
    stats = {"a": PhoneStats(mean=0.0, std=1.0, sample_size=100)}
    score = calibrate_score(raw_value=-4.0, phone="a", phone_stats=stats)
    assert score < 5


def test_raw_value_far_above_mean_gives_high_score():
    stats = {"a": PhoneStats(mean=0.0, std=1.0, sample_size=100)}
    score = calibrate_score(raw_value=4.0, phone="a", phone_stats=stats)
    assert score > 95
...
def test_score_is_clamped_to_0_100_range():
    ...
    high = calibrate_score(raw_value=100.0, phone="a", phone_stats=stats)
    low = calibrate_score(raw_value=-100.0, phone="a", phone_stats=stats)
    assert high == 100
    assert low == 0
...
def test_missing_phone_with_no_fallback_raises():
    ...
    with pytest.raises(ValueError):
        calibrate_score(raw_value=0.0, phone="ɑ̃", phone_stats=stats)
...
def test_undersampled_phone_without_fallback_uses_its_own_stats_anyway():
    ...
    score = calibrate_score(raw_value=0.0, phone="ɑ̃", phone_stats=stats, min_sample_size=30)
    assert score == 50


def test_zero_std_does_not_crash():
    stats = {"a": PhoneStats(mean=0.0, std=0.0, sample_size=100)}
    score = calibrate_score(raw_value=5.0, phone="a", phone_stats=stats)
