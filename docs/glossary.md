...
- **Anomaly detection framing (one-class)** — an alternative framing some MDD literature adopts explicitly: fit a model only on correctly-pronounced/native speech, then score how much an inference-time sample deviates from that "normal" distribution — treating mispronunciation as a statistical anomaly rather than something learned from labeled error data. Shahin & Ahmed (2019, *Speech Communication*, "Anomaly detection based pronunciation verification approach using speech attribute features") trained a One-Class SVM per phoneme on native speech's phonological (articulatory) features and reported it outperforming DNN-based GOP on disordered and foreign-accented speech; a 2023 follow-up applies the same OCSVM idea to wav2vec2-derived features. Paulou's own percentile/z-score calibration (D11) is a simple parametric instance of this same family — a per-phone univariate Gaussian one-class model fit on native `confidence`/`raw_gop` values, flagging deviation via z-score rather than a learned decision boundary. Noted here as a framing worth citing for the Publication Plan's contribution story — not a reason to change the MVP design.
