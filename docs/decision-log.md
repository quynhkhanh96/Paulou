...
**Status:** Locked in. Local verification (Piper phoneme injection, determinism, and case content) completed — see D44 for the verified case list, synthesis parameters, and word-boundary technique.

### D44 — Diagnostic audio set (D43) verified locally; final Tranche 1+2 case list, deterministic synthesis parameters, and word-boundary technique

**Decision:** D43's pending local verification is complete: Piper (a French voice, `fr_FR-siwis-medium`) runs successfully end to end via the phonemes_to_ids/phoneme_ids_to_audio path, producing distinguishable native-vs-perturbed audio confirmed by both duration measurement and direct listening. The diagnostic case list (`experiments/diagnostic_set/cases.py`) is finalized at 12 cases across two tranches: Tranche 1 (6 cases) covers Paulou's core liaison focus (D3/D9) — all three common liaison consonants (z, t, n) across obligatoire (les amis, petit ami), interdite (un héros for h-aspiré, et amis for the after-"et" rule), and a genuine two-boundary liaison chain (les anciens amis, reusing D23's own worked example); Tranche 2 (6 cases) covers broader, non-liaison error types in scope per D36's expanded framing — final-consonant deletion (table, porte), weak-cluster-internal deletion (spectacle), and substitution beyond the original single consonant case (nasal-vowel confusion in bonjour, oral-vowel confusions in lune and deux). Synthesis is pinned to `noise_scale=0.0, noise_w_scale=0.0` (removing Piper's built-in stochastic noise) and exposes a `--length-scale` CLI flag (default 1.3x slower) for easier manual review. A word-boundary pause token is inserted, at the phoneme-list level, between two words in whichever rendering (native or perturbed) does not have an intact liaison connecting them.

**Rationale:**
- The case list was expanded from an initial 3 cases to 12 after reviewing an external (Grok-authored) ~48-case proposal brought into the discussion. One concrete error in that proposal was caught and corrected before use: its "un petit ami" case was labeled a liaison "chain", but direct verification via espeak-ng showed "un" and "petit" have no liaison at all ("petit" is consonant-initial) — only one liaison boundary exists in that phrase, not two. The genuine chain example used instead, "les anciens amis", was not invented for this purpose — it is D23's own pre-existing worked example, directly verified via espeak-ng to produce two consecutive liaison consonants.
- The case list was deliberately split into two tranches rather than building the external proposal's full ~48 cases at once: Tranche 1 stays scoped to liaison, matching D9/D19/D36's actual concern; Tranche 2 picks 6 representative cases from the proposal's remaining categories (final-consonant deletion, weak-cluster deletion, broader substitution) rather than all ~20 of them, as a first pass sized for an MVP go/no-go signal rather than population-representative coverage (per D43's own already-stated scope).
- Determinism required an explicit fix, not just a default: Piper's VITS decoder samples Gaussian noise inside the ONNX graph itself (`noise_scale` for the decoder, `noise_w_scale` for the stochastic duration predictor), confirmed by reading `piper/voice.py::phoneme_ids_to_audio` directly. Left at voice-config defaults, two consecutive runs of the identical phoneme input produced different per-file durations; in one observed run, native and deletion renderings of "les amis" coincidentally landed on the exact same duration, erasing the expected signal entirely. Zeroing both parameters was confirmed, across two consecutive full runs, to produce byte-for-byte-consistent durations per file.
- The word-boundary technique follows directly from D35's existing rule (a pause belongs between separate PronunciationUnits, never within a liaison_group) applied to whichever rendering currently lacks liaison: deletion cases get the boundary in the perturbed rendering only (the native rendering keeps its real liaison, no gap); insertion cases get it in the native rendering only (the perturbed rendering wrongly sounds connected via the inserted consonant, so it gets none). This models how a learner would actually produce each error — someone unaware of a liaison treats the two words as unconnected; someone wrongly applying a liaison produces a connected-sounding pair.

**Tradeoff accepted:**
- The word-boundary technique has a confirmed failure mode: for `un_héros`, a pause immediately after the nasal vowel `œ̃` produces a nasal-release artifact that is perceptually similar to /n/ — confirmed by direct listening, and exactly backwards from what that case needs to demonstrate (no consonant in the native rendering). `et_amis` (an oral vowel precedes its boundary) does not show this problem. Rather than abandon the technique for all insertion cases, only `un_héros`'s native rendering was reverted to no boundary; the underlying phoneme data fed to `FreePhoneRecognizer` is unaffected either way (this technique only aids manual listening verification) — a future liaison-insertion case ending in a nasal vowel should avoid this same trap.
- Duration is confirmed unreliable as a manual-verification signal for longer, multi-word utterances: `les_anciens_amis`'s native-vs-deletion durations differed by only 0.02s (1.17s vs 1.15s) despite listening confirming the correct phoneme was missing — far smaller than the 0.10-0.16s gap seen in the short 2-word cases. Piper's duration predictor appears to partially compensate elsewhere in a longer sequence when one phoneme is removed. Listening, not duration, is the verification method to rely on for anything longer than a two-word case.
- Tranche 2's 6 cases are a judgment call on which sub-categories matter most, not full coverage of the external proposal; the remaining categories it raised (schwa insertion, isolated word-initial insertion, additional substitution pairs) stay available as a Tranche 3 if 12 cases don't produce a clear enough canonicalizer-bias signal once run against a real `FreePhoneRecognizer`.
